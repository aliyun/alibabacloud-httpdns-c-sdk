...
static httpdns_resolve_result_t *single_resolve_response_to_result(httpdns_single_resolve_response_t *response) {
    ...
    if (IS_BLANK_STRING(response->host)) {
        result->host = sdsnew(response->host);
    }
    if (IS_BLANK_STRING(response->client_ip)) {
        result->client_ip = sdsnew(response->client_ip);
    }
    if (IS_BLANK_STRING(response->extra)) {
...
static void on_http_finish_callback_func(char *response_body, int32_t response_status, int32_t response_rt_ms,
    ...
    NEW_EMPTY_LIST_IN_STACK(httpdns_resolve_result);
    if (resolve_request->using_multi) {
        ...
        httpdns_list_dup(&httpdns_resolve_result, &response->dns, DATA_CLONE_FUNC(single_resolve_response_to_result));
        ...
        httpdns_list_add(&httpdns_resolve_result, response, DATA_CLONE_FUNC(single_resolve_response_to_result));
        httpdns_single_resolve_response_destroy(response);
    }
    size_t resolve_result_size = httpdns_list_size(&httpdns_resolve_result);
    for (int i = 0; i < resolve_result_size; i++) {
        httpdns_resolve_result_t *resolve_result = httpdns_list_get(&httpdns_resolve_result, i);
...
int32_t httpdns_resolve_task_execute(httpdns_resolve_task_t *task) {
    ...
    for (int i = 0; i < resolve_context_size; i++) {
        ...
        resolve_param->request->query_type = query_dns_type;
...
int32_t httpdns_client_simple_resolve(httpdns_client_t *httpdns_client,
    ...
    httpdns_resolve_request_t *request = httpdns_resolve_request_create(httpdns_client->config, host, resolver, query_type);
