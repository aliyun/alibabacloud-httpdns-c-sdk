...
httpdns_client_t *httpdns_client_new(httpdns_config_t *config) {
    if (httpdns_config_valid(config) != HTTPDNS_SUCCESS) {
        httpdns_log_error("create httpdns client failed, config is invalid");
        return NULL;
    }
...
httpdns_resolve_task_t *httpdns_resolve_task_new(httpdns_client_t *httpdns_client) {
    if (NULL == httpdns_client) {
        httpdns_log_info("create httpdns resolve task failed, httpdns client is NULL");
        return NULL;
    }
...
int32_t httpdns_resolve_task_add_request(httpdns_resolve_task_t *task, const httpdns_resolve_request_t *request) {
    if (NULL == task || NULL == request) {
        httpdns_log_info("add resolve request into task failed, task or request is NULL");
        return HTTPDNS_PARAMETER_EMPTY;
    }
    if (httpdns_string_is_blank(request->account_id)) {
        httpdns_log_info("add resolve request into task failed, account_id is blank");
...
static void on_http_complete_callback_func(httpdns_http_context_t *http_context,
    ...
    httpdns_log_debug("on_http_complete_callback_func %s", http_context_str);
    httpdns_sds_free(http_context_str);

    if (NULL == user_callback_param) {
        httpdns_log_debug("user callback param is NULL, skip");
        return;
    }

    httpdns_on_http_finish_callback_param_t *param = (httpdns_on_http_finish_callback_param_t *) user_callback_param;
    ...
    if (http_context->response_status != HTTPDNS_HTTP_STATUS_OK) {
        httpdns_sds_t http_context_str = httpdns_http_context_to_string(http_context);
        httpdns_log_info("http response exception, httpdns_conxtext %s", http_context_str);
        httpdns_sds_free(http_context_str);
...
static char *determine_miss_query_type(httpdns_cache_entry_t *cache_entry, char *expected_query_type) {
    ...
    // 部分命中缓存，查找缺失结果
    if (httpdns_is_query_type_both(expected_query_type) && httpdns_list_is_empty(&cache_entry->ips)) {
        return HTTPDNS_QUERY_TYPE_A;
    }
    if (httpdns_is_query_type_both(expected_query_type) && httpdns_list_is_empty(&cache_entry->ipsv6)) {
...
static void on_http_finish_callback_param_free(httpdns_on_http_finish_callback_param_t *param) {
    if (NULL != param) {
        free(param);
    }
}
...
int32_t httpdns_resolve_task_execute(httpdns_resolve_task_t *task) {
    if (NULL == task || NULL == task->httpdns_client || httpdns_list_is_empty(&task->resolve_contexts)) {
        httpdns_log_info("httpdns execute resolve task failed, task or client or contexts is empty");
        return HTTPDNS_PARAMETER_EMPTY;
    }
    ...
    httpdns_list_for_each_entry(resolve_context_cursor, &task->resolve_contexts) {
        ...
        if (NULL == request->resolver) {
            httpdns_log_debug("fill resolver into request");
            request->resolver = httpdns_scheduler_get(scheduler);
        }
        if (httpdns_is_query_type_auto(request->query_type)) {
            httpdns_log_debug("unwrap auto query type");
        ...
        if (!request->using_multi && request->using_cache) {
            ...
            if (NULL == query_dns_type) {
                ...
                httpdns_log_debug("hit cache, cache_key %s, query_type %s, skip", request->cache_key, request->query_type);
                continue;
            }
        }
        ...
        httpdns_new_object_in_heap(on_http_finish_callback_param, httpdns_on_http_finish_callback_param_t);
    ...
    do {
        ...
        httpdns_list_for_each_entry(resolve_param_curosr, &resolve_params) {
            ...
            httpdns_on_http_finish_callback_param_t *on_http_finish_callback_param = resolve_param->user_http_complete_callback_param;
...
int32_t httpdns_client_simple_resolve(httpdns_client_t *httpdns_client,
    ...
    if (NULL == request || NULL == httpdns_client) {
        httpdns_log_info("httpdns client custom resolve failed, request or httpdns client is empty");
        return HTTPDNS_PARAMETER_EMPTY;
    }
    if (NULL == httpdns_client->config
        || httpdns_string_is_blank(request->host)
        || httpdns_string_is_blank(request->query_type)) {
        httpdns_log_info("simple resolve failed, config or host or query type is empty");
        return HTTPDNS_PARAMETER_EMPTY;
    }
